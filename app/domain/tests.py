...
The data-access seam for the four test modes (per-Testee spec, frozen,
hand-authored, benchmark) and the frozen/hand-authored question set.
Mode-specific *field* validation lives in the schema layer; structural
and lifecycle rules (campaign lock, publish gate, forward-only edit,
question-config shape) live here. Per-Testee generation and benchmark
sequencing run against the attempt (P4 Slice 2), not the test.
...
equality with Python-side ordering/pagination (catalogue precedent),
so the zero-DB/zero-network test seam holds (AC-CD15).

CODE_SPEC §3 locks the ``app/domain/*`` module list to eight names;
this module (with ``assignments``/``attempts``) is a P4 structural
addition folded into the phase handover per SESSION_START — the
structure-gate passes unmodified (it checks required paths exist; extra
domain modules are permitted) and the addition is well-rationalised
(test/assignment/attempt orchestration has no existing home; engagement
logic stays in the locked ``app/domain/engagement.py``).
"""
...
async def create_test(
    ...
    *,
    ...
    mode-field matrix and timing/pause rules; this sets explicit values
    for every non-nullable column (the test seam does not apply
    ``server_default``s — P3 trap)."""
    test = Test(
...
async def publish_test(db: AsyncSession, test: Test, *, actor_id: uuid.UUID) -> Test:
    """Draft → published. Frozen/hand-authored tests must carry at least
    one authored question (AC-D5)."""
...
def validate_question_config(qtype: QuestionType, config: Any) -> None:
    ...
    types must be gradeable without AI (P4 Slice 3); AI-graded types
    carry a rubric + model answer. Raises ``APIError(422)``."""
