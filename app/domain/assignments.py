...
system groups carry rule-derived membership (not stored) — resolution
of that rule lands here (the groups router deferred it to P4).

``engagement_status`` is intentionally derived, not stored — that read
-time derivation + the reminder/escalation sweep land in P4 Slice 3
(``app.domain.engagement``), querying attempts by ``Attempt.assignment_id``
per AC-D26 (v1.4). This module owns CRUD + the assignee snapshot only.
Routers stay thin (CODE_SPEC §2/§3).

This module is a P4 structural addition folded into the phase handover
per SESSION_START (see ``app.domain.tests`` for the rationale).
"""
...
async def _resolve_group_members(db: AsyncSession, group: Group) -> list[uuid.UUID]:
    ...
    groups read ``group_member`` rows (AC-D15)."""
    if group.is_system:
...
async def create_assignment(
    ...
    *,
    ...
    Individual targets are recorded first (``via_group_id`` null); group
    members follow, tagged with the group they were resolved through. A
    user targeted both ways is recorded once (the unique constraint on
    ``(assignment_id, user_id)``); the individual target wins."""
    ...
    if not targets:
        raise APIError(
            422,
            "no_assignees",
            "An assignment must resolve to at least one Testee.",
        )
...
async def withdraw_assignment(
    ...
    """Withdraw an assignment. Historical attempts are preserved — the
    migration-0004 FK is ``ON DELETE SET NULL``, so any attributed
    attempt's ``assignment_id`` is cleared rather than the row deleted,
    and engagement derivation then reads "no attributed attempts" for
    the withdrawn assignment (AC-D14 retention / AC-D26)."""
