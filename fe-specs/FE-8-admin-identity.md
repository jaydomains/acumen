...
- **New in this PR (identity scope):**
  ...
  - `MembersTable` — table per `admin-authoring.jsx:666–694`. Columns: Name, Email (mono), Last active (relative), Remove row action (danger). Filter member search above the table. **N2:** rows come from `GET /v1/groups/{id}/members` (`UserResponse`); the Joined column is dropped and Last active renders an em-dash placeholder until `joined_at`/`last_active_at` land on the wire.
...
| `GET /v1/groups/{group_id}/members?cursor={cursor}&limit=50` | Cursor-paginated member list — a single batched call that replaced the prior N+1 over `member_ids` × eager `useAdminUsers` (N2). Returns full `UserResponse` rows. | **Built (N2)** — returns `Page_UserResponse_`, mirroring `/v1/users`. Member search is client-side over the loaded page (no server `q`). There is no `joined_at`/`last_active_at` on the wire, so the "Last active" column renders an em-dash placeholder. |
| `POST /v1/groups/{group_id}/members` | Add a single member. Body: `{ user_id: string }` (NOT bulk). The picker multi-select fans out N parallel single-user POSTs via `Promise.allSettled`. | **Exists** — body schema `GroupMemberRequest`. |
