...
> **Amended in v1.9** — autonomous-content-generation cycle, **PR-C**. The human **approve/review-queue** gate is removed for **AI-generated** pills: under the autonomous pipeline (AC-D29 generation → AC-D30 self-review → AC-D31 auto-publish gate) generated pills **auto-publish** (publish-with-warning below the confidence threshold; nothing held pre-publish), and the incremental bootstrap fires on **auto-publish** rather than admin approval (**bootstrap-on-publish**, F1 — folded complete here per amend-once). **Reciprocal cross-reference with AC-D23** (closes the mutual pair AC-D23 opened in PR-A: AC-D23's bootstrap-on-publish reframe and this anchor's approve→auto-publish removal tell one approve→auto-publish story). The `pill_proposal` **refiner** + its `approve_pill_proposal` admin path are **retained** as the optional manual entry point (G7a) — bypassed for generated drafts, not deleted. Ratified through the authenticated in-session channel for this cycle. See the v1.9 paragraph in Implications.
