...
>
> **Decision count:** 34 decisions (AC-D1 through AC-D34; AC-D27 added in v1.2, AC-D28 + AC-D29 added in v1.9, AC-D30 + AC-D31 added in v1.9, AC-D32 + AC-D33 + AC-D34 added in v1.10). 6 amendments applied in v1.1 to AC-D4, AC-D9, AC-D11, AC-D18, AC-D19, and §8.7; 3 further amendments applied in v1.2 to AC-D9, AC-D22 (with §7.3), and AC-D25; 1 clarifying amendment applied in v1.3 to AC-D19 (review_status pending state); 1 clarifying amendment applied in v1.4 to AC-D26 (Attempt→Assignment attribution link); 1 clarifying amendment applied in v1.5 to AC-D3 (sequence_number scope per Testee per Test); 6 amendments applied in v1.6 (pre-build spec-audit consolidation) to AC-D4, AC-D9, AC-D11, AC-D12, AC-D19, and AC-D26, plus technical-anchor updates to AC-CD8 and the AC-CD11 gate-checklist; 1 amendment applied in v1.7 (AC-CD11 P6 gate closure) to AC-D19 — submit-wait wording realigned to the batched / 60-s-ceiling contract (F10 resolved) — plus the AC-CD11 technical anchor itself moving from deferred to resolved (mode + latency ceiling locked); 1 amendment applied in v1.8 (AC-CD10 / §10 P10 build gate closure) to AC-D25 — execution model locked as in-process `asyncio.gather` + `asyncio.Semaphore` (Celery wording retired from the user-facing path); Question gains an attempt-scoped `attempt_position` column for stable streamed-arrival ordering; single-failure policy locked as one orchestration-layer retry then AC-D11 pause; **AC-D28 minted and 3 amendments applied in v1.9** (autonomous-content-generation amendment cycle, **PR-A — corpus & authority foundation**): **AC-D28** (source-authority allowlist + tiered scoring) minted; **AC-D21** (web search extended to corpus acquisition; self-review re-adjudicates `safety_relevant`; admin tag-override relocated to retroactive oversight), **AC-D22** (Drive-folder RAG ingestion retired in favour of the AI-built reference corpus; "queried at every generation call" extended to §6.5), and **AC-D23** (Drive embed bootstrap step retired → reference-corpus build; incremental bootstrap fires on auto-publish; mutual cross-reference with AC-D7) amended — all ratified through the authenticated in-session channel for this sequenced ratification cycle (workstream PR #107, detail-plan PR #108, extraction PR #109). **AC-D29 minted and the AI-operation count amended in v1.9** (autonomous-content-generation amendment cycle, **PR-B — generation + provenance + ops-count**): **AC-D29** (generation provenance chain) minted; the AI-operation count moves **seven → nine** — `pill_generation` (Anthropic-family, B1) and `content_self_review` (cross-family, C1; protocol AC-D lands in PR-C) — touching **AC-D1** Implications, **AC-CD8** (enum prose + numeral), and **AC-D12** (Anthropic five→six, cross-family two→three); built-state note: nine ops are canonical, `content_self_review` wiring completes in PR-C. **AC-D30 + AC-D31 minted and AC-D7 amended in v1.9** (autonomous-content-generation amendment cycle, **PR-C — auto-publish gate + self-review + governance prose**): **AC-D30** (generated-content self-review protocol — multi-pass cross-model, the safety floor) and **AC-D31** (autonomous auto-publish gate — single global threshold + publish-with-warning) minted; **AC-D7** amended (human approve/queue gate removed for generated pills → auto-publish; incremental bootstrap fires on auto-publish per F1; reciprocal cross-reference with AC-D23 closed); SPEC §6.5 rewritten to the autonomous pipeline, §6.9 content-self-review op added, §290 audit-log records published / publish-with-warning / rolled-back. Cycle remains **v1.9** (one version per sequenced cycle). **AC-D32 minted in v1.10** (content-pipeline-maturity Gate-2 amendment chain, **AMD-A** — dual-path generation: `llm_direct` / `corpus_grounded` modes, llm-direct primary for the KBC pilot, corpus-grounded matured for future tenants); AMD-A carries the v1.9→v1.10 version bump across the paired docs (one version per cycle — the remaining AMD-B…AMD-K amendments of this chain stay v1.10). **AMD-C amends AC-D30 + AC-D31 in v1.10** (confidence math — `llm_direct` confidence derived from cross-model self-review agreement per DDP-14, + `PublishRecord.confidence` recompute-on-demote, discharging audit P2-#9). **AMD-E amends AC-D28 in v1.10** (retrieval-side demotion — `retrieve_corpus_for_topic` excludes `denied` `DemotedSource` hosts, discharging audit P1-#2). **AC-D33 minted in v1.10** (content-pipeline-maturity **AMD-G** — per-tenant generation-mode configuration: per-tenant `generation_mode` + optional per-topic override per DP-1, per-tenant allowlist scoping per DP-3; KBC defaults to `llm_direct`). **AC-D34 minted in v1.10** (content-pipeline-maturity **AMD-I** — inter-source contradiction detection + surfacing: genuine T1-vs-T1 contradiction on the same claim detected via distance-prefilter + LLM cross-check per DDP-17, surfaced to the oversight/flag layer per DP-11, discharging audit P2-#3). **AMD-K amends AC-D7 + AC-D31 (and verifies AC-D23) in v1.10** (gated-pilot reconciliation, DP-4/CA-4 — the admin `/approve` queue gates *entry* to the auto-publish gate while the gate still holds nothing once invoked; the informed-approval CA-2/DDP-27 gate-signal-exposure requirement is anchored; autonomy enables post-FE-10; bootstrap-on-publish fires on the `/approve`-triggered publish identically — discharging audit P1-#3). **AMD-J amends AC-D18 in v1.10** (cost posture visibility-only → enforcing — a hard monthly-spend kill-switch + per-sweep batch ceiling per DDP-4, both-AI-keys-fail-closed-outside-dev per DDP-5, and publish-health metrics + ≥1 alert; discharging audit P2-#2/P2-#10/P2-#7; no AC-CD7 cron-count touch). All ratified through the authenticated in-session channel for this cycle. Original v1.0/v1.1 wording preserved in git history; current document reflects amended decisions as the authoritative text.
...
| AC-CD8 | (technical, CODE_SPEC §7) operation enum drives 9-operation → 4-method routing (v1.9: +`pill_generation`→generate, +`content_self_review`→review); provenance persists on every AI-produced entity via the shipped AI-provenance columns |
...
**Implications:** Acumen owns **nine** AI-driven operations as of v1.9 (test generation, grading, weakness identification, learning material generation, pill proposal, grade review per AC-D19, anchor self-review per AC-D23, **pill generation per AC-D29 — the autonomous topic→N draft generator, distinct from the pill-proposal refiner**, and **content self-review per AC-D30 — the cross-model generated-content review floor**); the original seven were locked at v1.1, pill generation + content self-review added in the v1.9 autonomous-content cycle. *Built-state note: nine operations are canonical; `pill_generation` wiring lands with B1 execution and `content_self_review` (protocol per AC-D30) wiring lands with C1 execution.* The `embed` operation remains the un-counted **tenth** enum member (canonical: nine named operations + the internal `embed` call, which is not a user-facing AI operation). Each is a separate version-controlled prompt. Codebase root follows the standalone repo's conventions. Permission keys live under `acumen:*`. SiteMesh-specific architectural commitments (MeshCore posture, substrate role, inter-module events) are deferred to the conversion-phase spec.
...
> **Amended in v1.9** — autonomous-content-generation cycle, **PR-C**. The human **approve/review-queue** gate is removed for **all AI pill drafts** (autonomous *and* the refiner-polished manual path): under the autonomous pipeline (AC-D29 generation → AC-D30 self-review → AC-D31 auto-publish gate) AI pills **auto-publish** (publish-with-warning below the confidence threshold; nothing held pre-publish), and the incremental bootstrap fires on **auto-publish** rather than admin approval (**bootstrap-on-publish**, F1 — folded complete here per amend-once). **Reciprocal cross-reference with AC-D23** (closes the mutual pair AC-D23 opened in PR-A: AC-D23's bootstrap-on-publish reframe and this anchor's approve→auto-publish removal tell one approve→auto-publish story). The `pill_proposal` **refiner endpoint** is **retained** (G7a — the "admin drafts a name + description, AI polishes" workflow), but its `approve_pill_proposal` human gate is **removed entirely**: refiner-polished drafts route through the **same AC-D31 auto-publish gate** as generated drafts — one publication path, no per-source gate exception. Ratified through the authenticated in-session channel for this cycle. See the v1.9 paragraph in Implications.

> **Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-K** (gated-pilot reconciliation, CA-4; ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-K, DP-4). The v1.9 "approve gate removed entirely; governance shifts from pre-publish approval to retroactive oversight" below is the **autonomous-mode** design. Under the **DP-4 gated pilot** that autonomous mode is **disabled for v1**: an admin **`/approve` entry-gate is re-introduced** as the pilot's compensating control — generated (and refiner-polished) drafts **queue** as `pending` `pill_generation` rows and an admin approves each before it publishes through the (unchanged) AC-D31 gate. This is **not** a reversal: the pre-publish-autonomous-gate-removal and retroactive-oversight governance remain the **target posture, enabled when FE-10's oversight/rollback surface lands** (the next workstream); the pilot simply **does not open** the autonomous path until that control exists, running the admin `/approve` queue in the interim. **Bootstrap-on-publish (AC-D23 reciprocal) is unaffected:** the incremental bootstrap fires on the **publish event** whether triggered autonomously (future) or by an admin `/approve` (pilot) — `auto_publish_draft` is the single publish path both converge on, so AC-D23's trigger holds under the gated pilot.
...
**Implications:** Pills are versioned entities with metadata (difficulty range, estimated time, related pills, `safety_relevant` flag per AC-D21, `safety_links` cached set for safety pills per AC-D21, anchor question pools per band per AC-D20). New AI pill drafts — autonomous per AC-D29, or the retained `pill_proposal` refiner per G7a — carry a self-applied `safety_relevant` classification per AC-D21 (re-adjudicated by the AC-D30 self-review) and flow through the **AC-D31 auto-publish gate** rather than an admin review queue; the incremental bootstrap fires on **auto-publish** per AC-D23 (bootstrap-on-publish, F1). *(Admin may still create/seed pills directly per the hybrid governance above.)* Admin can manually retire pills at any time; retired pills are hidden from active flows but retained per AC-D14. Admin can group pills into named Learning Paths. Testees can also create personal paths from pills they select.

**(v1.9)** Under the autonomous pipeline (§6.5), **AI-generated** pills are **not** routed through an admin review queue: the AC-D31 auto-publish gate publishes each generated draft autonomously (publish, or publish-with-warning below the confidence threshold; nothing held pre-publish, subject to the NS-7 degrade rule for single-provider safety-relevant content), after the AC-D30 multi-pass cross-model self-review re-adjudicates `safety_relevant`. The "approved → incremental bootstrap" trigger is reframed: the incremental bootstrap fires on **auto-publish** per AC-D23 (bootstrap-on-publish, F1). The **`pill_proposal` refiner endpoint** (admin supplies name+description, AI polishes) is **retained** as the optional manual entry point (G7a), but its `approve_pill_proposal` human gate is **removed entirely** — its polished output routes through the **same AC-D31 auto-publish gate** as generated drafts (one publication path, no per-source gate exception). Admin governance shifts from **pre-publish approval** to **retroactive oversight** (the Stage-E dashboard + rollback — AC-CD26, PR-D). This anchor and **AC-D23** carry reciprocal cross-references (the mutual pair: AC-D23 = bootstrap-on-publish; AC-D7 = approve-gate removal).

**Spec reference:** §3, §4, §5, §6.5.

**Related:** AC-D5, AC-D6, AC-D8, AC-D14, AC-D20, AC-D21, AC-D23, AC-D29, AC-D30, AC-D31.
...
**Decision:** v1.1 defaults to Claude Sonnet for the Anthropic-side AI operations (generation, grading, weakness identification, learning material, pill proposal — **six as of v1.9, adding `pill_generation`**) and OpenAI for the cross-family operations (grade review per amended AC-D19, anchor self-review per AC-D23 — **three as of v1.9, adding `content_self_review`** per the self-review AC-D, PR-C). Admins may override the default per operation in system settings. Model overrides are recorded against each AI-produced entity for traceability. Specific stakes-based overrides may be configured per Test.
...
> **Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-J** (ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-J; DDP-4 ruled **A**, DDP-5 ruled **A**). Discharges audit **P2-#2 (HIGH)**, **P2-#10 (MEDIUM)**, **P2-#7 (MEDIUM)**. The autonomous content pipeline (AC-D29/AC-D30/AC-D31) makes the v1 "no hard enforcement" posture (item 2 / the v1.x deferral below) unsafe at scale, so this anchor moves from **visibility-only to enforcing** for the autonomous-generation spend path:
>
> **Hard cost ceiling (DDP-4 A — both controls).** Alongside the retained 50/80/100% alerts, a **hard monthly-spend kill-switch** halts new autonomous generation/publish when `current_month_spend ≥ monthly_ai_budget` (a stop, distinct from the alert — it guards total/sustained spend), **and** a **per-sweep batch ceiling** caps the generation batches opened per gap-detection / catalogue-health sweep run (it guards single-sweep burst fan-out — the audit's flooded-`GapSignal` case a monthly switch would not catch until after the burst fired). The two are complementary, not redundant. The hard cap is a **precondition of any future autonomous enablement** (the v1 pilot is gated regardless — DP-4 / AC-D31 AMD-K). **No new scheduled cron** — the batch ceiling lives inside the existing gap-detection / catalogue-health sweeps and the kill-switch rides the off-cron worker-task drain (P1.1), so the AC-CD7 nine-cron count is **untouched**.
>
> **Missing-key fail-closed (DDP-5 A — both keys, unconditional).** Outside `DEV_ENVS`, an unset required AI key — **both** `anthropic_api_key` and `openai_api_key`, always — makes `check_startup_config` return an **error** so boot **fails closed**, rather than the prior warning that let a prod deploy silently serve the stub provider and publish stub content. Startup config validation is kept **unconditional** (not coupled to the runtime mode/NS-7 path): the AC-D30 cross-model review floor must be *configurable* at boot; NS-7 single-provider degrade remains a distinct **runtime** path for a provider failing mid-operation.
>
> **Publish-health observability (P2-#7).** The cost surface gains **publish-health metrics** — publish-rate, flag-rate (`low_confidence` share), confidence-distribution, drain-backlog-depth — plus **≥1 proactive alert** on a defined anomaly (flag-rate or backlog over a threshold). Emission is **fail-soft** (never blocks or fails a publish — the `maybe_fire_budget_alert` "never raises" contract).
>
> The per-Testee rate limits (item 3) and the cost dashboard (item 1) are **unchanged**; this adds enforcement + observability, it does not relax the limits. The cap config, kill-switch fn, fail-closed promotion, metrics module, and alert hook are P1.3 / P1.6 execution deliverables; the metrics-sink (DDP-7) and alert-channel (DDP-8) mechanisms are execution choices.
...
Hard budget enforcement, graceful model degradation, and per-pill or per-subject budgets are explicitly deferred to v1.x. *(Superseded in part by AMD-J v1.10, above: a hard monthly-spend **kill-switch** and a **per-sweep batch ceiling** now enforce the autonomous-generation spend path; **graceful model degradation** and **per-pill / per-subject budgets** remain deferred — neither is in the audit blocker set nor pilot-gating.)*
...
> **Verified in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-K** (gated-pilot reconciliation). Bootstrap-on-publish fires on the **admin-`/approve`-triggered publish under the DP-4 gated pilot** identically to autonomous publish — `auto_publish_draft` is the single publish path both the (disabled-for-v1) autonomous drain and the pilot `/approve` queue converge on, so this anchor's "fires on auto-publish" trigger holds unchanged under the gated pilot. The AC-D7↔AC-D23 reciprocal pair stays consistent across the DP-4 posture change (no trigger reframe; only the *upstream* enablement is gated — AC-D7 / AC-D31 AMD-K).
...
> **Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-E** (ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-E). The per-source demotion (the E2 source-override layer described below) is now consulted at **retrieval** as well as acquisition: `retrieve_corpus_for_topic` (AC-CD25) **excludes corpus chunks whose `source_host` is a `denied` `DemotedSource`**, not merely filtering by `min_tier`. This closes the audit **P1-#2 (HIGH)** gap — previously a demoted host's **already-stored** chunks stayed in the retrieval pool and the next generation could re-ground new pills on the discredited source (demotion blocked re-acquisition but not re-use). Acquisition-side demotion (`filter_demoted`) is **unchanged**; the effective rule is that a `denied` source is excluded from **both** acquiring new chunks and retrieving existing ones, so E2 demotion and A3 retrieval compose symmetrically. *(The retrieval-side exclusion mechanism — an in-SQL join on `DemotedSource` vs a post-query filter — is the P3.3/DDP-18 execution choice; the anchor rule is that retrieval excludes denied hosts.)*
...
**Per-source override / demotion (E2 half — DS13-a ruled multi-slice [A1+E2]).** This anchor also governs the retroactive per-source rollback surface: the oversight dashboard (AC-CD26, PR-D) may **demote or retract** a previously-acquired source. The demotion is recorded in a **DB-level source-override layer keyed by `source_host`** that overrides the code-registry tier at retrieval/ranking time — the code allowlist supplies the seed/default tier, the DB override supplies any operator demotion; the effective source-authority signal is the **join** of the two. The override layer's schema lands with the oversight rollback contract in **PR-D** — **completed there (DS13-a = A):** realized as the `demoted_sources` source-override table (`source_host`, `denied`/`tier_override`, `reason`, `actor_id`) in the **AC-CD26** oversight rollback contract, which AC-D28's `is_allowlisted`/`authority_tier` checks consult on top of the code seed; `rollback_source` writes a demotion so the corpus builder skips the host. This anchor recorded the design in PR-A so the chain carried the multi-slice [A1+E2] shape from the outset; the schema completes that design (no re-decision — the [A1+E2] amend-once closes here).
...
**Related:** AC-D28 (source authority), AC-CD25 (corpus builder), AC-D21 (safety), AC-D7 (catalogue); the auto-publish-gate **AC-D31** (PR-C) + the oversight read/rollback **AC-CD26** (PR-D, which queries this store by `source_host` for per-source rollback).

---

## AC-D30 — Generated-content self-review protocol

> **Minted in v1.9** — autonomous-content-generation cycle, **PR-C** (Slice C1). Ratified through the authenticated in-session channel (detail-plan PR #108 Slice 7; extraction PR #109 §B AM-10). NS-2 ruled **one new op + three variant passes**; NS-7 ruled **degrade-not-gate**.

> **Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-C** (ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-C). For AC-D32 `llm_direct` drafts (which carry no corpus authority — AC-CD27), the cross-model self-review **agreement** this protocol produces is the **primary** confidence signal that AC-D31's `compute_confidence` consumes (in place of authority-weighted provenance). The three-pass protocol, the cross-provider requirement, and the **NS-7 degrade-not-gate** rule are otherwise **unchanged** and apply identically across both generation modes (AC-D32). *Rationale: a draft grounded in the model's parametric knowledge has no AC-D28 source-tier to weight, so the orthogonal cross-model agreement signal — already produced by this protocol — is the honest confidence primary; degrading it to a flat base would make the publish threshold non-discriminating on the KBC pilot's primary path (DDP-14 ruled A).*

**Decision:** Every AI-generated pill draft (AC-D29) passes a **multi-pass, cross-model self-review** before the auto-publish gate (AC-D31) acts on it — the non-negotiable safety floor (ruling 4). One `Operation.content_self_review` carries **three prompt-variant passes**: **grounding/factual** (are the draft's claims supported by the cited corpus chunks / `GenerationProvenance`? → verdict + unsupported-claim list), **safety** (is `safety_relevant` correctly classified, and is any safety-teaching content present? → **re-adjudicated `safety_relevant`** + verdict), and **provenance** (does every claim resolve to a corpus source, no orphan claims? → verdict + orphan list). Each pass runs on a **different provider from the generator** (Anthropic generates per AC-D29 → OpenAI reviews), extending the AC-D19 / AC-D23 cross-provider pattern. The safety pass **re-adjudicates** `safety_relevant` — the autonomous replacement for AC-D21's removed pre-publish admin catch on a false-negative mis-tag.

**NS-7 — degrade-not-gate (ratified).** Cross-model is the floor, but a single-provider deployment cannot run it. Single-provider **safety-relevant** content **publishes-with-warning** (always dashboard-flagged) on same-model multi-pass review **+ a "single-provider verified" flag** — degrade, **no** second-provider prerequisite gate (honours ruling 2's "nothing held"). C1 exposes the degradation switch (default `degrade`) that the AC-D31 gate reads.

**Rationale:** With no human pre-publish gate, self-review is the autonomous quality + safety floor. Cross-model review produces orthogonal signal (different inductive task, not same-family homophily) — the AC-D19 grade-review / AC-D23 anchor-review precedent. Per-assertion grounding + provenance (AC-D29) make each pass checkable against the actual cited sources rather than against model intuition.

**Implications:** `Operation.content_self_review` minted (joins `_REVIEW_DEFAULT_OPS`, OpenAI cross-model default; the four op-keyed maps + the three `_VARIANT_REGISTRY` passes are C1 execution deliverables). A `self_review_draft(db, *, draft, provenance) -> SelfReviewResult` domain fn runs the three passes and returns the three verdicts + the re-adjudicated `safety_relevant`; the AC-D31 gate consumes them into the confidence score + publish decision. Distinct op from `anchor_self_review` (reviews a *generated pill draft* against grounding/safety/provenance, not an *anchor question* against quality criteria) — reusing it would conflate contracts + cost/provenance aggregation (NS-2).
...
## AC-D31 — Autonomous auto-publish gate

> **Minted in v1.9** — autonomous-content-generation cycle, **PR-C** (Slice C2). Ratified through the authenticated in-session channel (detail-plan PR #108 Slice 8; extraction PR #109 §B AM-11). Rulings 1 (single global threshold) + 2 (publish-with-warning, nothing held); NS-6 threshold default + formula; DS8-a `PublishRecord`.

> **Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-C** (ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-C). Two changes, both riding AC-D32 dual-path generation:
>
> **(1) Mode-aware confidence (DDP-14 ruled A; P2.5).** `compute_confidence` branches on `generation_mode` (AC-D32): `corpus_grounded` retains the authority-weighted-provenance + corroboration math below **unchanged**; `llm_direct` (no `authority_scores`, AC-CD27) derives confidence from a **graded function of cross-model self-review agreement** (AC-D30 — the fraction of self-review passes agreeing, scaled), replacing the flat `_NO_GROUNDING_BASE = 0.5`. Cross-model agreement is the *primary* signal **only where a second provider is configured**; under the AC-D30 NS-7 rule a single-provider safety-relevant `llm_direct` draft cannot run cross-model and **degrades to publish-with-warning + `single_provider_verified`** (degrade-not-gate, unchanged). The threshold stays a **single global** `pill_publish_confidence_threshold` across modes (DP-2 — re-evaluate to mode-specific only if dashboard telemetry warrants).
>
> **(2) Recompute-on-demote (P3.5; discharges audit P2-#9).** `PublishRecord.confidence` is **no longer frozen-immutable** at publish: when a grounding source is **demoted** (AC-D28 / AC-CD26 `rollback_source`), the confidence + the source-authority breakdown of the **live pills grounded on that source** are **recomputed** through the same `compute_confidence`, so a demote-without-rollback no longer leaves stale (possibly inflated) confidence on live content. (`llm_direct` pills are unaffected — they ground on no external source. The recompute *mechanism* — synchronous vs enqueued — is the P3.5 execution choice, DDP-20; the *rule* that confidence is recomputed when a grounding source is demoted is this anchor change.)

> **Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-K** (gated-pilot reconciliation, CA-4; ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-K, DP-4). **Pilot posture (DP-4 — gated).** For the **v1 KBC pilot**, autonomous publish is **disabled**: the generation→self-review→publish **drain is wired but not scheduled** (the wire-vs-enable invariant), and generated drafts accumulate as **`pending` `pill_generation` rows forming an admin approval queue**. An admin **`/approve`s** a draft, which then publishes through **this same gate** (`auto_publish_draft`). So under the pilot the **approval queue gates *entry* to** the auto-publish gate (an upstream hold) — but **the gate itself still holds nothing once invoked**: the `compute_confidence` → publish / publish-with-warning semantics above are unchanged, and the **"nothing held pre-publish" invariant is preserved for the future autonomous path** (enabled when FE-10's retroactive oversight surface lands, the next workstream). **Informed-approval requirement (CA-2 / DDP-27).** Because the `/approve` queue is the pilot's **only** compensating safety control (FE-10 deferred), the queue **must surface the publish-gate signals** on each pending draft — `confidence`, the `low_confidence` reason (below-threshold / `safety_failed` / NS-7 degrade), and `single_provider_verified` — so an admin approves **informed**, not blind; a list-and-click approve without the flag reasons would re-create the audit's Pass-1 Axis-10 inoperable-control gap DP-4 exists to close. The queue-endpoint shape (DDP-26) and the `/approve` row-lock (P1.2) are P5.4 execution deliverables. This gates the autonomous design's **enablement** for the pilot; it does not weaken the design.

**Decision:** For each AI-generated draft (AC-D29) that has passed self-review (AC-D30), Acumen computes a **confidence score** and **publishes with no human step**: **≥ a single global threshold → publish live**; **< threshold → publish-with-warning** (live + a dashboard `low_confidence` flag); **nothing is held pre-publish** (rulings 1 + 2), including safety-relevant content (subject to the AC-D30 NS-7 degrade rule). This **replaces the `approve_pill_proposal` human gate entirely** (AC-D7 amended) — **both** autonomously-generated and refiner-polished drafts auto-publish through this gate; `approve_pill_proposal` is removed, with no per-source gate exception. `compute_confidence(self_review, provenance) -> 0..1` combines the AC-D30 verdicts (a hard fail on grounding or provenance floors the score) with **authority-weighted provenance** (mean tier-score of the grounding chunks, AC-D28 DS1-a: T1=1.0/T2=0.6/T3=0.3) + the cross-source corroboration count (AC-CD25 DS2-b ≥(ii)). The threshold is a **single global** `SystemSettings.pill_publish_confidence_threshold` (**default 0.70**, per-tenant tunable — NS-6, **not** per-type per ruling 1).

**Rationale:** "Balls to the wall, rein in if it breaks" — autonomous publication with **retroactive** oversight, not a pre-publish gate. A single global threshold keeps the gate simple and auditable; publish-with-warning surfaces low-confidence content for retroactive review (Stage E) rather than blocking it; per-type telemetry from day one gives the data to re-evaluate to a per-type threshold *iff* it proves warranted (ruling 1's escape hatch).

**Implications:** `SystemSettings.pill_publish_confidence_threshold` (default 0.70) + migration; an `auto_publish_draft(db, task)` domain fn (run self-review → `compute_confidence` → publish or publish-with-warning via `create_pill`, honouring AC-D30's re-adjudicated `safety_relevant`; audit `pill_generation.publish` / `.publish_flagged`); a **`PublishRecord`** row per publish (`pill_id`, `batch_id`, `confidence`, per-pass verdicts, `low_confidence`, per-type telemetry — DS8-a) as the Stage-E read + per-batch-rollback surface. The incremental bootstrap fires on this publish event (AC-D7 / AC-D23 bootstrap-on-publish, F1). The `pill_proposal` refiner's drafts route through the same gate (G7a). The Stage-E oversight dashboard + rollback (AC-CD26, PR-D) consume `PublishRecord`. Migration + fns are C2 execution deliverables.
...
## AC-D32 — Dual-path generation: `llm_direct` and `corpus_grounded` modes

> **Minted in v1.10** — content-pipeline-maturity amendment cycle, **AMD-A** (dual-path generation modes). Ratified by the spec author through the authenticated in-session channel for this Gate-2 amendment chain (parent plan **PR #130** R0–R4 — dual-path direction; detail-plan **PR #131** §4 AMD-A + DP-6 anchor-surface confirmation; structural input `plans/AUDIT_OVERPASS_2026-06-13.md` "Fix-workstream scope preview"). First anchor of the v1.10 cycle — carries the v1.9→v1.10 version bump across the paired docs.

**Decision:** Acumen's autonomous pill generation (AC-D29 / §6.5) runs in one of **two generation modes**:

- **`corpus_grounded`** — the established path: drafts are grounded in the AI-built reference corpus (AC-CD25 / amended AC-D22), each claim bound to its grounding chunk(s)/source(s) through the per-assertion provenance chain (AC-D29), authority-weighted by source tier (AC-D28). The path the v1.9 autonomous-content workstream built.
- **`llm_direct`** — drafts are generated **directly from the model's parametric knowledge**, with **no reference-corpus grounding step**; the cross-model self-review protocol (AC-D30) is the quality + safety floor, and confidence derives primarily from **cross-model agreement** rather than corpus authority. Its provenance representation (a synthetic, non-corpus shape) is defined in **AC-CD27** (AMD-B); its confidence math is the AC-D30/AC-D31 amendment **AMD-C**.

**`llm_direct` is the primary mode for the KBC pilot; `corpus_grounded` is retained and matured as infrastructure for future tenants.** Mode is **selected per tenant** (with an optional per-topic override) through the per-tenant mode-configuration surface **AC-D33** (AMD-G); **KBC defaults to `llm_direct`**. Both modes feed the **same** downstream contract — the AC-D30 self-review floor and the AC-D31 auto-publish gate act on drafts from either mode identically; only the **grounding step, the provenance shape, and the confidence inputs** differ by mode.

**Rationale:** The v1.9 overpass audit (`AUDIT_OVERPASS_2026-06-13.md`, Pass 2 Axis 10) found the corpus-grounded path's *production reality* under-defended exactly where the KBC pilot's highest-authority sources live: SABS/SANS/`*.gov.za` standards are frequently paywalled or scanned-image PDFs, which acquisition silently mis-grounds (a login/preview page stamped T1, or an empty-text PDF falling through to ungrounded general knowledge). Maturing that path (content-validity gate, contradiction surfacing, retrieval-side demotion — Phase 3) is real work that need not block the pilot. **LLM-direct generation with cross-model self-review** is the more robust *pilot* content path: it depends on no external-corpus acquisition, while the cross-model review floor (AC-D30) and the auto-publish gate (AC-D31) still bound quality and safety. The corpus-grounded path is **not removed** — it is retained and matured for tenants whose content demands source-bound provenance. This is the **R0–R4 strategic direction** ratified with the spec author at the parent plan (PR #130 §1).

**Implications:** A **`generation_mode`** discriminator is carried on the generation unit (`pill_generation` `ProcessingTask`, resolved from the per-tenant AC-D33 config; column + routing are P2.1/P2.2 execution deliverables). `generate_grounded_drafts` branches on mode: `corpus_grounded` retains the AC-D29 corpus-retrieval + grounding step; `llm_direct` skips corpus retrieval and emits the AC-CD27 synthetic provenance. Self-review (AC-D30) and gate (AC-D31) are **mode-agnostic** in structure; the **confidence inputs** differ (corpus authority is unavailable in `llm_direct`, so cross-model agreement is the primary signal — AMD-C amends AC-D30/AC-D31 to define both). `PublishRecord` logs `generation_mode` so cross-mode comparability is evaluable from dashboard data (DP-2; the publish threshold stays a single global value across modes initially). No mode is auto-selected by the autonomous builder — mode is operator configuration (AC-D33), consistent with the "rein in if it breaks" posture. **No new `Operation` enum value** (mode reshapes the generator's inputs, not the AI-operation set; the nine-operation count is unchanged).

**Spec reference:** §6.5, §6.8.

**Related:** AC-D29, AC-D30, AC-D31, AC-D33 (AMD-G), AC-CD27 (AMD-B), AC-D28, AC-CD25.

---

## AC-D33 — Per-tenant generation-mode configuration

> **Minted in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-G**. Ratified through the authenticated in-session channel (detail-plan **PR #131** §4 AMD-G; DP-1 per-tenant + per-topic override settled; DP-3 allowlist scoping settled). The configuration surface AC-D32 references for selecting a tenant's generation mode. Blocks execution slice P4.3.

**Decision:** A tenant's **generation mode** (AC-D32 — `llm_direct` / `corpus_grounded`) is **per-tenant configuration** with an **optional per-topic override** (DP-1): each tenant carries a default `generation_mode`, and a specific topic/subject may override it. **KBC defaults to `llm_direct`** (R1 / AC-D32). The mode is **operator configuration**, never auto-selected by the autonomous builder (the AC-D32 "rein in if it breaks" posture).

**Multi-tenant allowlist scoping (DP-3).** The source-authority allowlist (AC-D28) is likewise **per-tenant-scoped**: a **shared public base** (the seeded T1/T2/T3 hosts) **plus per-tenant private additions** — a tenant may add private/authoritative hosts for its own content without affecting other tenants, and the shared base cannot be removed (additive per-tenant, the AC-D28 env-extension pattern generalised to per-tenant).

**Rationale:** Dual-path generation (AC-D32) only earns its keep if each tenant rides the mode its content warrants — KBC on `llm_direct` for the pilot, a future source-bound tenant on `corpus_grounded`. Per-tenant config with a per-topic override allows mixing modes within a tenant (e.g. a corpus-grounded tenant using `llm_direct` for a topic with no good corpus) without a global flag-day. Per-tenant allowlist scoping (DP-3) keeps one tenant's private authoritative sources out of another's grounding while preserving the shared public base. The per-tenant-mode-configuration direction ratified at the parent plan (PR #130 R1).

**Implications:** A per-tenant `generation_mode` setting (resolved by the AC-D32 routing at generation time; the setting + optional per-topic override are P2.1/P4.3 execution deliverables) and an admin surface to read/set it (the surface form — dedicated REST endpoint vs per-tenant `system_settings` vs FE-scoped — is the P4.3/DDP-22 execution choice). The allowlist gains per-tenant private additions on top of the shared base (AC-D28 / `source_authority.py`, P4.3). Tenant resolution rides the AC-CD3 `tenant_id` seam + the AMD-H tenant-threading (P4.1); RLS stays a port-seam. `PublishRecord.generation_mode` (DP-2) already logs the resolved mode. No new `Operation`; no AI cost.

**Spec reference:** §6.5, §6.8.

**Related:** AC-D32, AC-D28, AC-CD3, AC-CD27, AC-CD5.

---

## AC-D34 — Inter-source contradiction detection and surfacing

> **Minted in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-I**. Ratified through the authenticated in-session channel (detail-plan **PR #131** §4 AMD-I; DP-6 confirmed a dedicated anchor; DP-11 surface-to-oversight settled; DDP-17 ruled **C**). Discharges audit **P2-#3 (HIGH)**. Blocks execution slice P3.2.

**Decision:** When two **authoritative (T1) sources** in the reference corpus carry a **genuine semantic contradiction on the same claim** — a conflicting assertion about the same fact, not merely topical/embedding distance — the disagreement is **detected** and **surfaced to the retroactive-oversight / flag layer** (DP-11), rather than both chunks being silently retrieved and grounded into the same generation (the audit P2-#3 gap). A contradicted claim does not silently pass the AC-D30 grounding pass on the strength of *one* supporting source while another authoritative source contradicts it.

**Detection mechanism (DDP-17 ruled C — cheap-prefilter + accurate-confirm).** A cheap **embedding-distance prefilter** over the already-stored T1 corpus-chunk embeddings (reusing the AC-CD25 corroboration machinery) narrows candidate pairs; an **LLM cross-check** then confirms whether a low-similarity T1 pair is a *genuine contradiction on the same claim* versus merely *different sub-topics or differently-worded non-conflicting facts*. The two stages are required by the **CA-3 false-positive bar**: a distance-only rule over-flags (low cosine also covers different sub-topics) and would flood the oversight layer; the LLM confirm distinguishes contradiction from topical distance. The cross-check rides the existing cross-model review pattern (AC-D30) as a variant pass — **no new `Operation`; the nine-op count is unchanged** (AC-CD8) — and cost is bounded because the LLM runs only on the distance-prefiltered candidate pairs.

**Surfacing (DP-11).** A confirmed contradiction is recorded as a **contradiction facet on the oversight read surface (AC-CD26)** and flags the affected pill (a dashboard signal analogous to AC-D31 `low_confidence`), so an admin can rein it in retroactively — consistent with the "nothing held pre-publish" posture (AC-D31), not a block. A genuinely corroborating pair (AC-CD25 `corroboration_count`) and two T1 sources that merely cover different sub-topics are **not** flagged (no regression on corroboration; the CA-3 false-positive bar).

**Rationale:** The v1.9 audit (Pass 2 Axis 9 / P2-#3) found no contradiction handling — two T1 sources that disagree embed far apart, so neither is corroborated nor flagged, and both are independently retrieved and injected into the generation prompt; the AC-D30 grounding pass checks whether each claim is supported by *some* source, not whether sources mutually conflict. For SANS/NRCS content with superseded standard versions or differing numeric limits, that is a real correctness/safety gap. Surfacing the disagreement to retroactive oversight (DP-11) makes it visible and rein-in-able rather than silently resolved in the model's favour.

**Implications:** Extend the AC-CD25 corroboration logic (`corpus_builder.py:262–267`) with the T1-vs-T1 distance prefilter; an LLM cross-check variant (AC-D30 pattern, no new op) confirms; the oversight read surface (AC-CD26) gains a contradiction facet; publish flags the affected pill. Corpus-path concern — applies to `corpus_grounded` mode; `llm_direct` (AC-D32) grounds on no corpus and carries no inter-source contradiction. Not pilot-blocking (Phase 3, future-tenant). All P3.2 execution deliverables.

**Spec reference:** §6.5, §6.9.

**Related:** AC-D28, AC-D29, AC-D30, AC-D31, AC-CD25, AC-CD26.

---

*End of Acumen decisions anchor log. 34 decisions locked (AC-D1–AC-D34); 6 v1.1 + 3 v1.2 + 1 v1.3 + 1 v1.4 + 1 v1.5 + 6 v1.6 + 1 v1.7 + 1 v1.8 + (1 mint AC-D28 + 3 amendments AC-D21/AC-D22/AC-D23 in v1.9, autonomous-content cycle PR-A) + (1 mint AC-D29 + the AI-operation-count amendment to AC-D1/AC-CD8/AC-D12 in v1.9, autonomous-content cycle PR-B) + (2 mints AC-D30/AC-D31 + AC-D7 amendment in v1.9, autonomous-content cycle PR-C) + (1 mint AC-D32 in v1.10, content-pipeline-maturity cycle AMD-A) + (AC-D30 + AC-D31 amended in v1.10, content-pipeline-maturity cycle AMD-C) + (AC-D28 amended in v1.10, content-pipeline-maturity cycle AMD-E) + (1 mint AC-D33 in v1.10, content-pipeline-maturity cycle AMD-G) + (1 mint AC-D34 in v1.10, content-pipeline-maturity cycle AMD-I) + (AC-D7 + AC-D31 amended + AC-D23 verified in v1.10, content-pipeline-maturity cycle AMD-K) + (AC-D18 amended in v1.10, content-pipeline-maturity cycle AMD-J) plus new AC-D27. Status: v1.10. Paired with `SPEC.md` v1.10.*
