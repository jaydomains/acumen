...
- **`list_admin_queue` N+1 query pattern (P11 / scale-driven).** Slice 3 Gitar review flagged this as a `💡` (suggestion, not blocker) — for each row where `routed_to_admin = True`, four sequential equality lookups execute (`_attempt_by_id`, `_assignment`, `_pill`, `_weakness_report_pills`). Gitar's own framing: "acceptable for now... will need a JOIN-based query if the admin queue ever grows (e.g., bulk failure events)." The pattern matches the AC-CD15 harness constraint (single-column equality wheres only) and the v1 scale assumption (most reports route to autonomous mode so the queue is short). When operational data shows the admin queue routinely exceeds ~50 rows, swap in a SQL JOIN behind a `Protocol` boundary so the harness's equality-only walk remains the test seam — same pattern P11 would use for the cost-dashboard `loop_driven` cost-share aggregation.
