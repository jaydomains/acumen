...
**Audit Log** — Cross-cutting record of significant state transitions: grade overrides, autonomous loop decisions reversed, autonomous pill drafts **published / published-with-warning / rolled-back** per AC-D31 (and, for the retained `pill_proposal` refiner path, proposals approved or rejected), pill retirements, user role changes, deactivations, manual deletions, campaign lock/unlock events per AC-D24, escalation notifications per AC-D26, bootstrap runs per AC-D23, safety-pill link drift detections per AC-D21. Lightweight in v1 — captures actor, action, target entity, and timestamp. Important for the SiteMesh port where audit becomes a platform concern.
...
1. **Signal capture.** Coverage-gap signals are recorded as Testees and generation surface them: recent generated questions + their pill tags, Testee discovery searches that returned no good match, and admin scope-clarification signals. These persist in the **GapSignal** entity (§5 — landed in the signal-spine link, PR-D).
...
**Retained manual path (G7a).** The legacy **`pill_proposal` refiner** operation — admin supplies a name + description and the AI polishes it (the `approve_pill_proposal` path) — is **kept as an optional manual entry point**; its output routes through the same auto-publish gate (AC-D31). The autonomous pipeline is the default; the refiner is the manual override. *(Refiner inputs: admin-supplied name/description/subject/difficulty; output: a single polished draft with self-applied `safety_relevant` per AC-D21.)*

**Quality bar:** every published pill is corpus-grounded (AC-D29), self-reviewed cross-model (AC-D30), and confidence-scored (AC-D31); low-confidence and single-provider-safety content is published-with-warning and surfaced for retroactive oversight (§4.11), never silently dropped.
