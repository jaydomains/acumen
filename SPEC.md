...
**Pilot enablement (DP-4 — gated; AC-D31 / AC-D7).** For the **v1 KBC pilot** the *autonomous* publish above is **gated, not live**: the drain that would auto-run steps 3–5 is **wired but not scheduled**, and generated drafts queue as `pending` rows for an **admin `/approve`** (which surfaces the gate signals — `confidence`, the `low_confidence` reason, `single_provider_verified` — so approval is informed) before publishing through the same gate. The approval queue gates *entry* to the gate; the gate itself still holds nothing once invoked. The autonomous path described above **enables when FE-10's retroactive oversight surface lands** (the next workstream).
...
*End of Acumen specification. Status: v1.10. Paired with `DECISIONS.md` v1.10 (34 decisions; 6 v1.1 + 3 v1.2 + 1 v1.3 + 1 v1.4 + 1 v1.5 + 6 v1.6 + 1 v1.7 + 1 v1.8 amendments plus new AC-D27, + the v1.9 autonomous-content cycle PR-A: AC-D28 minted + AC-D21/AC-D22/AC-D23 amended, AC-CD25 minted; PR-B: AC-D29 minted + the AI-operation count amended seven→nine; PR-C: AC-D30 + AC-D31 minted + AC-D7 amended + §6.5 autonomous-pipeline rewrite; v1.7 closed the AC-CD11 P6 gate; v1.8 closes the AC-CD10 P10 build gate; + the v1.10 content-pipeline-maturity Gate-2 chain: AMD-A AC-D32 dual-path generation modes, AMD-B AC-CD27 LLM-direct provenance shape, AMD-G AC-D33 per-tenant mode config, AMD-I AC-D34 inter-source contradiction detection).*