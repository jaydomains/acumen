...
| Crons scheduled (7 of canonical 9) | P11 | AC-CD7 | `app/beat_schedule.py`, `app/worker.py` | built | `tests/integration/test_p11_beat_schedule.py::test_beat_schedule_has_exactly_seven_entries` (CODE_SPEC §8 verbatim); `::test_beat_schedule_each_entry_has_task_and_schedule` (crontab shape); `::test_beat_schedule_celery_app_carries_schedule` (`make_celery` wires the dict); `::test_celery_app_registered_tasks_include_all_seven` (registry); `tests/unit/test_p11_celery_wrappers.py` (7 wrapper smoke tests). **(v1.9, AC-CD7):** the canonical cron count is now **nine** — `corpus.refresh` (replacing `drive_rag.ingest`, A3) + `gap_detection.sweep` + `catalogue_health.check` (D4) are added by the autonomous-content-generation workstream below; the "exactly-seven" test reflects current built state and is updated to nine by those execution slices (not by this amendment). |
...
| Reference corpus builder (acquire → extract → embed) | A2 | AC-CD25; AC-D22; AC-D28; AC-CD1 | `app/models.py` (`CorpusChunk` + migration), `app/domain/corpus_builder.py` (NEW), `requirements.txt` (bs4 + pypdf), `app/ai/cost.py` | missing | not built — gated on PR-A merge + A1 merged + A2 execution. |
...
| Generation + provenance + ops-count | B1–B3 | AC-D29 (provenance); AC-D1/AC-CD8/AC-D12 ops-count (→nine); AC-CD25; AC-D9 (G3 min/max) | `app/ai/provider.py` (`Operation.pill_generation`), `app/ai/prompts/pill_generation.py`, `app/models.py` (`GenerationProvenance` + migration), `app/domain/` (`generate_grounded_drafts`, `enqueue_generated_drafts`), `app/ai/cost.py` (`_pill_generation_spend`) | missing | **amendment ratified (PR-B, v1.9); execution not built** — B1 `pill_generation` op (Anthropic; **built-op count seven→eight (8 of canonical 9), code enum 8→9** — spec prose was authored to the final nine in this PR-B amendment under the ratified B-1=A combined mode, so B1 execution adds the op, it does **not** re-touch spec prose); B2 corpus-grounded gen + `GenerationProvenance` (relational, per-assertion) at prompt v1.1.0; B3 N-draft fan-out + cost-share + `batch_id`. `content_self_review` (the ninth op) wiring completes in PR-C. |
| Self-review + auto-publish gate + governance | C1–C2 | AC-D30 (self-review); AC-D31 (auto-publish gate); AC-D7 (approve-gate removed); AC-D28/AC-D29 | `app/ai/provider.py` (`Operation.content_self_review`), `app/ai/prompts/content_self_review_*.py` (3 variants), `app/domain/` (`self_review_draft`, `compute_confidence`, `auto_publish_draft`), `app/models.py` (`PublishRecord` + migration), `app/config.py` (`pill_publish_confidence_threshold`) | missing | **amendment ratified (PR-C, v1.9); execution not built** — C1 `content_self_review` op + 3 cross-model passes (NS-7 degrade); C2 `compute_confidence` (default threshold 0.70, authority-weighted) + auto-publish gate + `PublishRecord`. Replaces `approve_pill_proposal` for generated drafts; `pill_proposal` refiner retained (G7a). |
| Signal spine + gap-detection/health crons + oversight | D1–D4, E1–E2, F1 | GapSignal §5 (D1–D2); §6.5/NS-4 (PR-C, D3); AC-CD7 nine-cron (PR-A, D4); **AC-CD26** (E1 read + E2 rollback); AC-D28/DS13-a `demoted_sources` (E2); AC-D7/AC-D23 bootstrap-on-publish (PR-C/PR-A, F1) | missing | **amendment ratified (PR-D, v1.9 — the final link); execution not built.** D1–D2 `GapSignal` polymorphic store + discovery-miss/question-tag capture + signal-layer dedup (`scope_clarification` type defined, capture deferred — signal-3); D3 `gap_detection_sweep` + `catalogue_health_check` (NS-4, third dedup arm) → `enqueue_generated_drafts`; D4 the two crons (8–9 of nine, already counted PR-A); E1 oversight read API; E2 `rollback_pill/question/batch/source` + `override_safety_relevant` + `demoted_sources` source-override layer; F1 wires bootstrap to the auto-publish event (reuse-only). Oversight admin FE (`fe-specs/FE-10-admin-oversight.md`) authored, build deferred. **Sequencing guard (ratified): D1–D2 must NOT execute until PR-C merges** — §6.5 (which the signal stores feed) is authored in PR-C. |
