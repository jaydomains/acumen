...
**Confidence:** confident default.
**Amended in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-H** (ratified through the authenticated in-session channel; detail-plan PR #131 §4 AMD-H; DDP-21 ruled A). The autonomous-content workstream domain functions thread `tenant_id` **from the actor** — a per-fn `tenant_id` parameter (DDP-21 A: most local, testable, leak-surface auditable from the signature) — rather than reading the hard-coded `SEED_TENANT_ID` literal the v1.9 build embedded across ~8 modules (`generation.py`, `gap_detection.py`, `bootstrap.py`, `oversight.py`, `publish.py`, `corpus_builder.py`, `source_authority.py`, `signals.py`). This closes the audit **P2-#6 (MEDIUM)** cross-tenant-leak exposure before a second tenant onboards (pre-second-tenant prep, not a live pilot bug). **RLS remains the deferred port-seam** — this is **application-layer** tenant threading, not row-level security; the single `acumen` schema and the `tenant_id`-on-every-table shape are unchanged. Existing single-tenant behaviour is preserved (KBC resolves to `SEED_TENANT_ID` through the actor seam; a regression test confirms the pilot is unaffected). The threading is a P4.1 execution deliverable.
...
this anchor.
**Confidence:** confident default.
**Reconciled in v1.10** — content-pipeline-maturity Gate-2 chain, **AMD-H**. The "**one-file swap** at port" intent above governs the **auth mechanism** (the role-check / deactivation / privacy-ack dependency in `permissions.py` + `auth.py`) — it does **not** claim that **tenant scoping** is single-file. The audit's Pass-2 Axis-2 noted tenant scoping spread across ~8 modules; that is **AC-CD3's** application-layer `tenant_id` threading (AMD-H), a **separate seam** from the auth swap. The two are orthogonal: swapping the auth mechanism to the SiteMesh Auth Hub remains a one-file change, while tenant resolution rides the AC-CD3 per-fn-threading seam. `require_role` continues to check role; tenant-membership authz (when a second tenant exists) rides the AC-CD3 seam, not this anchor. No commitment is walked back — this disambiguates two previously-conflated concerns.
...
> **Minted in v1.10** — content-pipeline-maturity Gate-2 amendment chain, **AMD-B**. Ratified through the authenticated in-session channel (detail-plan **PR #131** §4 AMD-B + DDP-13 ruled **A**; parent plan **PR #130** dual-path direction). Defines the provenance representation for the AC-D32 `llm_direct` mode, which grounds in the model's parametric knowledge rather than the reference corpus — so the AC-D29 corpus claim→source chain does not apply. Blocks execution slice P2.4.

**Decision.** A draft generated in `llm_direct` mode (AC-D32) writes **no `GenerationProvenance` rows** — there is no reference-corpus chunk to bind a claim to, so the AC-D29 per-assertion claim→corpus-source chain is **empty by construction**, not missing. The quality + safety evidence for an `llm_direct` draft is the **AC-D30 cross-model self-review verdicts** (recorded on `PublishRecord`, AC-D31); its confidence derives from **cross-model agreement** (AMD-C amends the AC-D30/AC-D31 confidence math) rather than authority-weighted corpus provenance (AC-D28, unavailable here). `PublishRecord.generation_mode` (DP-2) records which mode produced the draft so the oversight surface can distinguish the two provenance shapes.

**Oversight read degradation (discharges the audit P1-#11 tail).** The AC-CD26 oversight read facets resolve cleanly for an `llm_direct` pill: `item_provenance` returns an **explicit empty/`llm_direct` facet** ("generated from model knowledge, cross-model self-reviewed" — **not** an error or crash), and `source_authority_breakdown` **omits** `llm_direct` publishes from the authority-tier aggregation (they carry no `authority_tier`) rather than counting them tier-less. `rollback_source` / per-source rollback has no effect on `llm_direct` pills — correctly, since they grounded on no external source; `rollback_pill` / `rollback_batch` (keyed on `PublishRecord` / `batch_id`, not `GenerationProvenance`) still retract them.

**Rationale.** The simplest honest representation: a draft grounded on no corpus source carries **no** corpus provenance, not a synthetic row masquerading as grounding. A fabricated "model_training" row (DDP-13 option B, rejected) would inject a non-source into the same `GenerationProvenance` table the per-source rollback and authority-breakdown joins were built around (AC-D29 §5.1), risking it being counted in the authority radar — the opposite of the rein-in honesty the oversight surface exists for. Empty-by-construction keeps the corpus-provenance store meaning exactly "corpus-grounded claims," and avoids coupling AMD-B to the nullable-`corpus_chunk_id` migration (that nullability is AMD-F's CASCADE→SET NULL change, P3.4 — not load-bearing for `llm_direct` here).

**Implications.** `app/domain/generation.py`: the `llm_direct` branch (AC-D32 routing, P2.2) writes drafts + `PublishRecord` with `generation_mode='llm_direct'` and **skips** the `GenerationProvenance` write path entirely. `app/domain/oversight.py` (`item_provenance` / `source_authority_breakdown`, ~`:202,232,269`): confirm the empty-chain degradation is clean (the P2.4 acceptance tail). `PublishRecord` gains `generation_mode` (a P2.4 model+migration deliverable, DP-2). **No** `GenerationProvenance` schema change and **no** dependency on the nullable `corpus_chunk_id` column under this option. **Confidence:** confident default.
